...
export const dateFormatterBooking = (date: any): string => {
  ...
  console.log(format(anyDate, "yyyy-MM-dd HH:mm:ss.SSS"));
...
export const timeExtractor = (date: any): string => {
  ...
  return dateFormatted.format(anyDate);
};
...
export const dateIsAfter = (dateTarget: Date, dateToCompare: Date): boolean => {
  if (dateTarget > dateToCompare) {
    return true;
  } else {
    return false;
