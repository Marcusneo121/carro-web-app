...
interface MyHostingProps {
  hosting: IMyBooking;
}

const HostingCarCard = ({ hosting }: MyHostingProps) => {
  ...
  return (
    <div className="group flex flex-col items-center justify-center rounded-3xl bg-primaryblue-100 px-10 py-4 hover:bg-white hover:shadow-md">
      ...
      <div className="mt-5 flex w-full flex-row justify-between">
        <div>
          <h2 className="text-2xl font-extrabold">{hosting.car_name}</h2>
          ...
          <h3 className="mt-1 w-max rounded-lg text-lg font-semibold">
            <span className="text-2xl font-bold">
              RM {hosting.last_bargain_amount}
            </span>{" "}
            /day
          </h3>
        </div>
        <div className="flex flex-col items-end">
          <BookingStatusBadge
            badgeID={hosting.bargain_status_id}
            badgeType={hosting.ori_bargain_name}
          />
          {hosting.bargain_status_id === 2 ||
          hosting.bargain_status_id === 4 ? (
      ...
      <Button
        ...
        onClick={() => {
          router.push(`myhost/${hosting.ori_bargain_id}`);
        }}
      >
        VIEW MORE
      </Button>
